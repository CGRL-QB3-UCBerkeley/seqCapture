...
then 
   echo "miniconda2 is not installed! please run bash Miniconda2-latest-MacOSX-x86_64.sh to install!"
   exit 1
fi 

CHECKPATH=morePackages/FLASH2/
if [ ! -d "$CHECKPATH" ] 
then
...
conda install -n seqCapture --file seqCapture_v0.0.1_osx64_install.txt
...
then
   ...
   printf "\n"
   cd morePackages
   tar -xzf FLASH2.tar.gz
   tar -xzf Rcorrector.tar.gz
   tar -xzf RepeatMasker.tar.gz
   tar -xzf Super-Deduper.tar.gz
###########flash2################
...
#########add seqCap path to ~/.bashrc ##########
cd ..
PWD=`pwd .`
if ! grep -q $PWD ~/.bash_profile
then
...
if ! grep -q $PERLLIBPATH ~/.bash_profile
then 
    echo "export \"PERL5LIB=$PERLLIBPATH:\$PERL5LIB\"" >>  ~/.bash_profile
else
...
then 
     ...
     cpanm -q  Bio::PopGen::Statistics  --force --notest   -l $PERLINSTALLBASE
fi
printf "\n"
...
echo "do not forget to run source ~/.bash_profile right after installation or start a new terminal session"
echo "to activate and use seqCature pipeline, run source activate seqCapture"
echo "to deactivate, run source deactivate"
printf "\n"
